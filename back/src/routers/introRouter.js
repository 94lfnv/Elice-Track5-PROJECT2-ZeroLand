...
// GET: 소개 페이지의 쓰레기 배출량 증가 데이터
const introData = async (req, res, next) => {
  try {
    const file_name = "1996to2020_trashData_tonPerDay.csv";
...
// GET: 연령대별 환경관심정도 데이터
const introData2 = async (req, res, next) => {
  try {
    const file_name = "국민환경의식조사원자료.csv";
    const csvPath = path.join(__dirname, "..", "data", file_name);
    const csv = readFileSync(csvPath, "utf-8");
    // 문자열을 줄바꿈으로 구분 => 배열에 저장
    const rows = csv.split("\r\n");
...
introRouter.get("/intro", asyncHandler(introData));
introRouter.get("/intro/2", asyncHandler(introData2));
